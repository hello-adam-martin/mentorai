...
**Agent** (`mentor_node: agent`, Domain layer): `id`, `domain` (the domain id it sits in),
...
- **Domain**: every agent's `allowed_actions` resolve to Governance actions, its `objects` resolve
  to object types, and its `domain` matches a domain in this layer.
...
**Validation is built into `MissionControl.html`.** Open it, pick a business folder, and a pass/fail
